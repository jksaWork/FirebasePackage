...
if (! function_exists('setDefaultHeaders')) {
	...
	if (! function_exists('getFirebaseSeriverKey')) {
		/**
		 * set Default headers array if no headers found
		 *
		 * @return array
		 */
		function getFirebaseSeriverKey()
		{
			if(config('server_key'))return config('server_key');
			else throw new Exception('ServerKey Error');
		}
	}
